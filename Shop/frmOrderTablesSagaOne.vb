Public Class frmOrderTablesSagaOne
    ...
    Private Sub frmOrderTables_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Using Conn As New SqlClient.SqlConnection(Fmain.LOCAL_CONN_STR)
            Using Comm As New SqlClient.SqlCommand
                ...
                Comm.CommandText = "select distinct g.id, g.code, g.name, g.sort_id, g.active, g.rgbhex from order_tablegroups g inner join order_tables o on g.id = o.group_id where g.active = 1 and o.active = 1"
                Conn.Open()
                Using greader As SqlClient.SqlDataReader = Comm.ExecuteReader()
                    While greader.Read()
                        Dim group As New OrderTableGroup With {.Id = greader.GetInt32(greader.GetOrdinal("id")),
                                                                  ...
                                                                  .RgbHex = greader.Item("rgbhex").ToString()
                                                                 }
                        ...
                        For Each btn As Control In tlpButtons.Controls
                            If btn.GetType() Is Reflection.Assembly.GetAssembly(btn.GetType()).GetType("System.Windows.Forms.Button") Then
                                If btn.Name = orderTable.Code Then
                                    ...
                                    btn.BackColor = IIf(orderTable.IsEmpty = True,
                                                        IIf(String.IsNullOrEmpty(grp.RgbHex), Color.LightGreen, ColorTranslator.FromHtml(grp.RgbHex)),
                                                        Color.OrangeRed)
                                    btn.ForeColor = IIf(orderTable.IsEmpty = True, ColorTranslator.FromHtml("#404040"), Color.White)
    ...
    Private Sub btnSwitchViews_Click(sender As Object, e As EventArgs) Handles btnSwitchViews.Click
